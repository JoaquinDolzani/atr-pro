    ...
    <div className="space-y-4 pb-8">
      ...
      {cert === "warn" && (
        <div className="bg-warn/15 border border-warn/50 text-warn rounded-xl p-3 text-sm flex gap-2 items-start">
          ...
          <p>Próximo a vencer: certificado médico con {certMonths} {certMonths === 1 ? "mes" : "meses"}. Renová antes que se venza.</p>
        </div>
      )}
      {cert === "bad" && (
        <div className="bg-destructive/15 border border-destructive text-destructive rounded-xl p-3 text-sm flex gap-2 items-start">
          ...
          <p className="font-semibold">CERTIFICADO VENCIDO — presentá uno nuevo para poder entrenar.</p>
        </div>
      )}
      {a.monthsOwed > 0 && a.monthsOwed < 3 && (
        <div className="bg-warn/15 border border-warn/50 text-warn rounded-xl p-3 text-sm">
          Adeudás {a.monthsOwed} mes(es). Regularizá pronto para evitar la suspensión.
        </div>
      )}

      {tab === "home" ? <HomeTab athlete={a} patch={patch} coachWa={db.coach.whatsapp} /> : <ProfileTab athlete={a} patch={patch} />}
    </div>
  );
}

function HomeTab({ athlete: a, patch, coachWa }: { athlete: Athlete; patch: (m: (a: Athlete) => Athlete) => void; coachWa: string }) {
    ...
    <div className="space-y-4">
      ...
      <section className="bg-card border border-border rounded-2xl p-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          {days.map((d, i) => {
            ...
            const iso = d.iso;
            const hasT = !!a.trainings[iso];
            const hasR = !!a.reports[iso];
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-lg text-xs relative flex items-center justify-center transition
                  ${isSel ? "bg-primary text-primary-foreground font-bold glow" : isToday ? "border border-primary text-primary" : "bg-secondary/40 hover:bg-secondary"}`}
              >
                {d.day}
                {(hasT || hasR) && (
                  <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 size-1 rounded-full ${hasR ? "bg-success" : isSel ? "bg-primary-foreground" : "bg-primary"}`} />
                )}
              </button>
            );
          })}
      ...
      {showReport && (
        <ReportModal
          ...
          onSave={(r) => { patch((x) => ({ ...x, reports: { ...x.reports, [selected]: r } })); setShowReport(false); }}
        />
      )}
    </div>
  );
      ...
      {report.photos.length > 0 && (
        <div className="flex gap-2 overflow-x-auto">
          {report.photos.map((p, i) => (
            <img key={i} src={p} alt={`captura ${i + 1}`} className="h-24 rounded-lg border border-border" />
          ))}
        </div>
      )}
...
function ReportModal({ date, existing, onClose, onSave }: { date: string; existing?: Report; onClose: () => void; onSave: (r: Report) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-card border-t sm:border border-border rounded-t-3xl sm:rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto p-5 space-y-3">
        ...
        <div>
          <label className="text-xs text-muted-foreground">Enlaces (Strava, Garmin, Coros...)</label>
          {links.map((l, i) => (
            <div key={i} className="flex gap-2 mt-1">
              ...
              <button onClick={() => setLinks(links.filter((_, j) => j !== i))} className="text-muted-foreground"><X className="size-4" /></button>
            </div>
          ))}
        ...
        <div>
          <label className="text-xs text-muted-foreground">Capturas / fotos (múltiples)</label>
          <button onClick={() => fileRef.current?.click()} className="mt-1 w-full border-2 border-dashed border-border rounded-lg py-3 text-sm flex items-center justify-center gap-2 text-muted-foreground hover:border-primary hover:text-primary">
            ...
            <div className="flex gap-2 mt-2 overflow-x-auto">
              {photos.map((p, i) => (
                <div key={i} className="relative shrink-0">
                  <img src={p} className="h-20 rounded-lg border border-border" />
                  <button onClick={() => setPhotos(photos.filter((_, j) => j !== i))} className="absolute -top-1 -right-1 bg-destructive rounded-full size-5 flex items-center justify-center"><X className="size-3" /></button>
                </div>
              ))}
            </div>
          )}
        ...
        <button
          onClick={() => onSave({ date, links: links.filter(Boolean), photos, km, timeMin, rpe, notes })}
          className="w-full bg-primary text-primary-foreground font-semibold py-2.5 rounded-xl glow"
...
function ProfileTab({ athlete: a, patch }: { athlete: Athlete; patch: (m: (a: Athlete) => Athlete) => void }) {
  const ar = activeRace(a);
  return (
    <div className="space-y-4">
      <section className="bg-card border border-border rounded-2xl p-4 flex items-center gap-4">
        <label className="cursor-pointer">
          {a.photo ? <img src={a.photo} className="size-16 rounded-full object-cover border-2 border-primary" /> :
          ...
          <input type="file" accept="image/*" hidden onChange={(e) => {
            ...
            const r = new FileReader(); r.onload = () => patch((x) => ({ ...x, photo: r.result as string })); r.readAsDataURL(f);
          }} />
        </label>
        <div className="flex-1 min-w-0">
          <input value={a.name} onChange={(e) => patch((x) => ({ ...x, name: e.target.value }))}
            className="w-full bg-transparent text-lg font-bold border-b border-border focus:border-primary outline-none" />
          <input value={a.email} onChange={(e) => patch((x) => ({ ...x, email: e.target.value }))}
            className="w-full bg-transparent text-xs text-muted-foreground border-b border-border focus:border-primary outline-none mt-1" />
        </div>
      </section>
      ...
      <section className="bg-card border border-border rounded-2xl p-4">
        ...
        <textarea value={a.objectives} onChange={(e) => patch((x) => ({ ...x, objectives: e.target.value }))}
          rows={4} className="w-full bg-input border border-border rounded px-3 py-2 text-sm" placeholder="Ej: Bajar de 35' en 10K." />
      </section>
